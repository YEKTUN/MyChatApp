 ...
 const Profile = () => {
  ...
  const uploadImageToStorage = async (uri: string) => {
    ...
    try {
      ...
      firestore().collection('users').doc(user?.uid).update({photo: downloadURL});
    ...
    <SafeAreaView className="flex-1 ">
      <LinearGradient
        ...
        <View className=" w-full h-full  gap-y-[50px] mt-10" style={{flex: 1}}>
          <View
            ...
            <View className='flex-1  justify-center items-center'>
              {imageUri ? ( <Image
                ...
                resizeMode='cover' className='rounded-full'
              />):(<ActivityIndicator />)}
           
              <TouchableOpacity activeOpacity={0.5} onPress={selectImageWithPermission} className='absolute top-[70px] right-11  rounded-full bg-black w-8 h-8 justify-center items-center'>
          ...
          <View style={{flex: 1}} className=" gap-y-2 pt-4   ">
            ...
            <View className="flex-row gap-x-4 justify-start items-start m-2">
              <Text>About :</Text>
              <Text className="text-white h-[200px] w-[200px]    ">{about}</Text>
        ...
        <View className=" w-full h-full    " style={{flex: 2}}>
          <View style={{flex: 2}} className='flex w-full items-center  justify-end absolute bottom-10'  >
         <TouchableOpacity onPress={logout} className=' w-[100px] h-[40px] justify-center  items-center mt-10 rounded-full bg-red-100 '> 
